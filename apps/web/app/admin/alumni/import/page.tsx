...
export default function ImportAlumniPage() {
  ...
  const handleUpload = async () => {
    ...
    setError(null);
    try {
      const res = await adminApi.importAlumni(file);
      setPreview(res.data);
      setStep("preview");
    } catch (err) {
  ...
  const handleConfirm = async () => {
    ...
    setError(null);
    try {
      const res = await adminApi.confirmImport(preview.import_id);
      setResult(res.data);
      setStep("done");
    } catch (err) {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div>
        ...
        <h1 className="text-xl sm:text-2xl font-bold text-neutral-800">Import Data Alumni</h1>
        <p className="text-sm text-neutral-500 mt-1">Upload file Excel berisi data alumni untuk diimport ke database.</p>
      </div>
      ...
      {step === "upload" && (
        <div className="bg-white rounded-2xl border border-neutral-100 p-6 shadow-sm space-y-5">
          ...
          <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm text-blue-700">
            ...
            <p>Kolom wajib: <strong>Nama Lengkap</strong>, <strong>NIM</strong>, <strong>Tahun Lulus</strong></p>
            <p className="text-xs mt-1 text-blue-500">Baris pertama harus berisi nama kolom persis seperti format di atas.</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-neutral-100 p-8 shadow-sm text-center space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 max-w-xs mx-auto">
            ...
            <div className="bg-amber-50 rounded-xl p-3">
              ...
              <p className="text-xs text-amber-600">Dilewati (Duplikasi)</p>
            </div>
          </div>
